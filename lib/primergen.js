...
non-self-complementary 0.0 0.0 0.*/
  ...
  this.makeObjectFromTableOne = function(table) {
    var superObject = {};
    for (var i =0;i < table.length ; i++) {
      var masterkey = table[i][0] + (table[i][4]) ? (table[i][4]) : "";
      superObject[masterkey] = {
        deltaH: parseFloat(table[i][1]),
        deltaS: parseFloat(myTextArray[i][2]), 
        deltaG: parseFloat(myTextArray[i][3]),
      };
    }
      return superObject;
  };
  this.nearestNeighborTable = makeObjectFromTableOne(this.TABLE1);
};
