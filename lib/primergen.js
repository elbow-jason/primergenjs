...
//
  this.calcMeltingTempCelsius = function(nearestNeighborPair) {
    var IDEALGASCONST = 1.987;
    var enthalpy = del;
    var concentrationPrimers = 0.25; // [primer] is optimally 0.1 - 0.5 uM where u is micro
    var entropy;
    var meltingTemp = enthalpy / (entropy + IDEALGASCONST * Math.log(concentrationPrimers)) -273.15;
    return meltingTemp;
    };
...
$pg.makeTableOne = function(table) {
    ...
    for (var i =0;i < table.length ; i++) {
      ...
      nnTableObject[key] = {
        nnSeqOne: table[i][0].replace('d', ''),
        nnSeqTwo: table[i][4] ? table[i][4].replace('d','') : "",
